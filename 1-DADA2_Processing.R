...
plotQualityProfile(fnFs[1]) #problems to show all together
plotQualityProfile(fnRs[1]) #more dramatic drop in reverse reads
#dev.off()
...
#filtered_out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, maxN=0, maxEE=c(2,5), truncQ=2, rm.phix=TRUE, truncLen=c(240,220), compress=TRUE, multithread=TRUE)
...
err_forward_reads <- learnErrors(fnFs, multithread=TRUE)
err_reverse_reads <- learnErrors(fnRs, multithread=TRUE)
...
#QC filtered reads
derep_forward <- derepFastq(fnFs, verbose=TRUE)
names(derep_forward) <- sample.names
derep_reverse <- derepFastq(fnRs, verbose=TRUE)
...
merged_amplicons <- mergePairs(dada_forward, derep_forward, dada_reverse,
                               derep_reverse, minOverlap=25, verbose=TRUE)
